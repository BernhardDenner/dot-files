...
PROMPT='
$(_user_host)${_current_dir} $(git_prompt_info) $(_gerrit_prompt) $(_ruby_version) $(kube_ps1)
%{$fg[$CARETCOLOR]%}▶%{$resetcolor%} '

PROMPT2='%{$fg[$CARETCOLOR]%}◀%{$reset_color%} '

RPROMPT='$(_vi_status)%{$(echotc UP 1)%}$(_vim_session) $(_git_time_since_commit) $(git_prompt_status) ${_return_status}%{$(echotc DO 1)%}'
...
# print "VIM" if this is a shell started from vim
function _vim_session() {
  if [[ -n $VIMRUNTIME ]] ; then
    echo "%{$fg[yellow]%}VIM%{$reset_color%}   "
  fi
}
